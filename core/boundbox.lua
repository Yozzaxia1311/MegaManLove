...
function imageOverlapsRect(x, y, w, h, data, x2, y2, w2, h2)
  if rectOverlapsRect(x2, y2, w2, h2, x, y, w, h) then
    ...
    for xi=math.clamp(math.round(x2-x), 0, neww), math.clamp(math.round(x2-x)+w2, 0, neww) do
      for yi=math.clamp(math.round(y2-y), 0, newh), math.clamp(math.round(y2-y)+h2, 0, newh) do
...
function imageOverlapsCircle(x, y, w, h, data, x2, y2, r2)
  if circleOverlapsRect(x2, y2, r2, x, y, w, h) then
    ...
    for xi=math.clamp(math.round(x2-x)-r2, 0, neww), math.clamp(math.round(x2-x)+r2, 0, neww) do
      for yi=math.clamp(math.round(y2-y)-r2, 0, newh), math.clamp(math.round(y2-y)+r2, 0, newh) do
        local _, _, _, a = data:getPixel(xi, yi)
...
function imageOverlapsImage(x, y, w, h, data, x2, y2, w2, h2, data2)
  if rectOverlapsRect(x2, y2, w2, h2, x, y, w, h) then
    ...
    for xi=math.clamp(math.round(x2-x), 0, neww), math.clamp(math.round(x2-x)+w2, 0, neww) do
      for yi=math.clamp(math.round(y2-y), 0, newh), math.clamp(math.round(y2-y)+h2, 0, newh) do
        for xi2=math.clamp(math.round(x-x2), 0, neww2), math.clamp(math.round(x-x2)+w, 0, neww2) do
          for yi2=math.clamp(math.round(y-y2), 0, newh2), math.clamp(math.round(y-y2)+h, 0, newh2) do
            local _, _, _, a = data:getPixel(xi, yi)
